...
OWA.report.prototype = {
  ...
  showAutoRefreshControl: function (options) {
    ...
    c.push(
      '<input type="radio" name="autorefresh" id="autorefresh-on-button" /><label for="autorefresh-on-button">On</label>'
    );
    c.push(
      '<input type="radio" name="autorefresh" checked="checked" id="autorefresh-off-button" /><label for="autorefresh-off-button">Off</label>'
    );
    c.push('</span>');
    ...
    jQuery(selector + ' > .autoRefreshControl > .buttons > #autorefresh-on-button').click(
      function () {
        that.startAutoRefresh();
      }
    );

    jQuery(selector + ' > .autoRefreshControl > .buttons > #autorefresh-off-button').click(
      function () {
  ...
  createTabs: function () {
    ...
    for (tab in this.tabs) {
      if (this.tabs.hasOwnProperty(tab)) {
        jQuery('#report-tabs > .report-tabs-nav-list').append(
          OWA.util.sprintf('<li><a href="#%s">%s</a></li>', tab, that.tabs[tab].label)
        );
      }
    }
  ...
  setupDomElements: function () {
    ...
    jQuery.get(OWA.getOption('modules_url') + 'base/templates/filter_period.php', function (tmpl) {
      ...
      jQuery('#owa_reportPeriodFilterSubmit').click(function () {
        jQuery(that.dom_id).trigger('owa_new_time_period_set', [
