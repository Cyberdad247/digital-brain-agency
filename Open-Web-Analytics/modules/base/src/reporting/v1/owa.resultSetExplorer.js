...
OWA.resultSetExplorer.prototype = {
  ...
  showLoader: function () {
    jQuery('#' + this.dom_id).append(
      '<div class="loader"><img class="loading" src="' +
        OWA.getSetting('baseUrl') +
        '/modules/base/i/loader.gif"></div>'
    );
  },
                ...
                for (var z in this.columnLinks[y].params) {
                  if (this.columnLinks[y].params.hasOwnProperty(z)) {
                    template = template.replace(
                      '%s',
                      OWA.util.urlEncode(
                        this.resultSet.resultsRows[i][this.columnLinks[y].params[z]].value
                      )
                    );
                  }
                }
  ...
  getSeries: function (value_name, value_name2, filter) {
    if (this.resultSet.resultsRows.length > 0) {
      ...
      for (var i = 0; i <= this.resultSet.resultsRows.length - 1; i++) {
        ...
        if (value_name2) {
          item = [
            this.resultSet.resultsRows[i][value_name].value,
            this.resultSet.resultsRows[i][value_name2].value,
          ];
        } else {
...
OWA.dimensionPicker.prototype = {
  ...
  generateDimList: function (selector, selected) {
    ...
    var c =
      '<select data-placeholder="Select..." name="dim-list" class="dim-list" style="width:150px;"><option value=""></option>';
    var that = this;
    ...
    jQuery(container_selector + ' > .dim-list')
      .chosen()
      .change(function () {
...
OWA.dataGrid.prototype = {
  generate: function (resultSet) {
    ...
    if (resultSet.page == 1) {
      jQuery(
        '#' + that.dom_id + ' > .owa_resultsExplorerBottomControls > UL > .owa_nextPageControl'
      ).show();
      jQuery(
        '#' + that.dom_id + ' > .owa_resultsExplorerBottomControls > UL > .owa_previousPageControl'
      ).hide();
  ...
  makeGridPagination: function (resultSet) {
    if (resultSet.more) {
      ...
      jQuery(
        '#' +
          that.dom_id +
          ' > .owa_resultsExplorerBottomControls > UL > .owa_previousPageControl > .ui-button-text'
      ).css('line-height', '0.5');
      // bind click
      ...
      jQuery(
        '#' +
          that.dom_id +
          ' > .owa_resultsExplorerBottomControls > UL > .owa_nextPageControl > .ui-button-text'
      ).css('line-height', '0.5');
      //bind click
      jQuery(
        '#' + that.dom_id + ' > .owa_resultsExplorerBottomControls > UL > .owa_nextPageControl'
      ...
      if (resultSet.page == 1) {
        jQuery(
          '#' +
            that.dom_id +
            ' > .owa_resultsExplorerBottomControls > UL > .owa_previousPageControl'
        ).hide();
      }
    }
  },
  ...
  displayRowCount: function (resultSet) {
    if (resultSet.total_pages > 1) {
      ...
      var check = jQuery(
        '#' + that.dom_id + ' > .owa_resultsExplorerBottomControls > UL > .owa_rowCount'
      ).html();
      //alert(check);
      if (check === null) {
        jQuery('#' + that.dom_id + ' > .owa_resultsExplorerBottomControls > UL').append(p);
  ...
  injectDomElements: function (resultSet) {
    var p = '';
    p +=
      '<div class="owa_genericHorizontalList explorerTopControls"><ul></ul><div style="clear:both;"></div></div>';
    ...
    jQuery('#' + that.dom_id + '_grid_secondDimensionChooser').bind(
      'dimension_change',
      function (event, oldname, newname) {
        ...
        jQuery('#' + that.dom_id).trigger('secondary_dimension_change', [
          that.previousDimensionName,
          newname,
        ]);
    ...
    // secondard dimension picker
    jQuery('#' + that.dom_id + ' > .explorerTopControls > ul').append(
      '<li class="controlItem"><span class="label">Filter:</span> <span class="constraintPicker"></span></li>'
    );
    // constraint builder selector
    var cb_button_selector =
      '#' + this.dom_id + ' > .explorerTopControls > ul > .controlItem > .constraintPickerButton';
    var cb_cont_selector =
      '#' + this.dom_id + ' > .explorerTopControls > ul > .controlItem > .constraintPicker';
