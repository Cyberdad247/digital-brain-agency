...
(function ($) {
  ...
  $.fn.extend({
    jqote: function (data, t) {
      ...
      this.each(function (i) {
        ...
        for (var j = 0; j < data.length; j++) dom += f.call(data[j], i, j, data, f);
      });
  ...
  $.extend({
    jqote: function (elem, data, t) {
      ...
      if (type === FUNC) fn = [elem];
      else if (type === ARR)
        fn =
          type_of.call(elem[0]) === FUNC
            ? elem
            : $.map(elem, function (e) {
                return $.jqotec(e, t);
              });
      else if (type === STR)
        fn.push(elem.indexOf('<' + t) < 0 ? $.jqotec($(elem), t) : $.jqotec(elem, t));
      else
        fn = $.map($(elem), function (e) {
          return $.jqotec(e, t);
        });

      for (var i = 0, l = fn.length; i < l; i++)
        for (var j = 0; j < data.length; j++) dom += fn[i].call(data[j], i, j, data, fn[i]);
    ...
    jqotec: function (elem, t) {
      var fn,
        ...
        type = type_of.call(elem),
        tmpl =
          type === STR && elem.indexOf('<' + t) >= 0
            ? elem
            : (elem = type === STR || elem instanceof jQuery ? $(elem)[0] : elem).innerHTML;
      ...
      return type_of.call(elem) === STR
        ? fn
        : ($.jqotecache[(elem.jqote = elem.jqote || n++)] = fn);
    },
