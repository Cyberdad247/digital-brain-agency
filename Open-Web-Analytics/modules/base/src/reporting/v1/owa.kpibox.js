...
OWA.kpiBox.prototype = {
  ...
  generate: function (resultSet, dom_id, options) {
    ...
    jQuery('#' + dom_id).append(
      OWA.util.sprintf(
        '<div id="%s" class="metricInfoboxesContainer" style="width:auto;"></div><div style="clear:both;"></div>',
        con_id
      )
    );
    ...
    for (var i in resultSet.aggregates) {
      if (resultSet.aggregates.hasOwnProperty(i)) {
        ...
        var html = OWA.util.sprintf(
          '<div id ="%s" class="owa_metricInfobox" style="min-width:135px;width:%s">',
          item.dom_id,
          width
        );
        html += OWA.util.sprintf('<p class="owa_metricInfoboxLabel">%s</p>', item.label);
        html += OWA.util.sprintf(
          '<p class="owa_metricInfoboxLargeNumber">%s</p>',
