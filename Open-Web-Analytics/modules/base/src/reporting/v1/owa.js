...
var OWA = {
  ...
  loadHeatmap: function (p) {
    var that = this;
    OWA.util.loadScript(
      OWA.getSetting('baseUrl') + '/modules/base/js/includes/jquery/jquery-1.6.4.min.js',
      function () {}
    );
    OWA.util.loadCss(
      OWA.getSetting('baseUrl') + '/modules/base/css/owa.overlay.css',
      function () {}
    );
        ...
        if (OWA.getSetting('hashCookiesToDomain')) {
          ...
          if (cookie_value.hasOwnProperty('cdh')) {
            ...
            if (cookie_value.cdh == dhash) {
              OWA.debug(
                'Cookie: %s, index: %s domain hash matches current cookie domain. Loading...',
                store_name,
                i
              );
              state = cookie_value;
              ...
              OWA.debug(
                'Cookie: %s, index: %s domain hash does not match current cookie domain. Not loading.',
                store_name,
                i
              );
            }
            ...
            OWA.debug(
              'Cookie: %s, index: %s has no domain hash. Not going to Load it.',
              store_name,
              i
            );
          }
    ...
    if (!key) {
      ...
      if (state && state.hasOwnProperty(key)) {
        ...
        this.replaceStore(
          ...
          state,
          true,
          this.getFormat(store_name),
          this.getExpirationDays(store_name)
        );
      }
    }
  },
...
OWA.util = {
  ...
  decodeCookieValue: function (string) {
    ...
    OWA.debug(
      'decodeCookieValue - string: %s, format: %s, value: %s',
      string,
      format,
