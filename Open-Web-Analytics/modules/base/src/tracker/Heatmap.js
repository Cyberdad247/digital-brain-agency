...
class Heatmap {
  ...
  showControlPanel() {
    ...
    jQuery('#owa_overlay').append(
      '<div class="owa_overlay_control" id="owa_overlay_start">Start</div>'
    );
    jQuery('#owa_overlay_start').toggleClass('active');
    jQuery('#owa_overlay').append(
      '<div class="owa_overlay_control" id="owa_overlay_stop">Stop</div>'
    );
    jQuery('#owa_overlay').append('<div class="owa_overlay_control" id="owa_overlay_end">X</div>');
    jQuery('#owa_overlay_start').click(function () {
      that.startTimer();
    });
    jQuery('#owa_overlay_stop').click(function () {
      that.stopTimer();
    });
  ...
  plotDotsRound(data) {
    for (var i = 0; i < data.length; i++) {
      ...
      if (data[i].x <= this.docDimensions.w && data[i].y <= this.docDimensions.h) {
        ...
        OWA_instance.debug(
          'not getting image data. coordinates %s %s are outside the canvas',
          data[i].x,
          data[i].y
        );
        continue;
      ...
      if (data[i].x >= 0 && data[i].y >= 0) {
        ...
        OWA_instance.debug(
          'not getting image data. coordinates %s %s less than zero.',
          data[i].x,
          data[i].y
        );
        continue;
      ...
      var rgr = this.context.createRadialGradient(
        data[i].x,
        data[i].y,
        7,
        data[i].x,
      ...
      this.context.fillRect(
        data[i].x - this.options.dotSize,
        data[i].y - this.options.dotSize,
        2 * this.options.dotSize,
        2 * this.options.dotSize
      );
  ...
  getDim(d) {
    ...
    if (d.body) {
      if (d.body.scrollHeight) {
        h = scr_h = d.body.scrollHeight;
        w = d.body.scrollWidth;
      }
      if (d.body.offsetHeight) {
        h = off_h = d.body.offsetHeight;
  ...
  createCanvas(w, h) {
    var that = this;
    jQuery('body').append(
      '<canvas id="owa_heatmap" width="' +
        w +
        'px" height="' +
        h +
        'px" style="position:absolute; top:0px; left:0px; z-index:999999;padding:0; margin:0;background: rgba(127, 127, 127, 0.5);"></canvas>'
    );
  }
