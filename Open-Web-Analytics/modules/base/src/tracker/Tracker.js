...
class OWATracker {
  ...
  setEndpoint(endpoint) {
    endpoint =
      'https:' == document.location.protocol
        ? window.owa_baseSecUrl || endpoint.replace(/http:/, 'https:')
        : endpoint;
          ...
          for (var i = 0; i < n; i++) {
            if (Util.is_object(properties[param][i])) {
              for (var o_param in properties[param][i]) {
                data[Util.sprintf(OWA.getSetting('ns') + '%s[%s][%s]', param, i, o_param)] =
                  properties[param][i][o_param];
              }
            } else {
  ...
  cdPost(data) {
    ...
    if (!iframe_container) {
      ...
      div.setAttribute(
        'style',
        'border: none; overflow-x: hidden; overflow-y: hidden; display: none;'
      );
  ...
  generateHiddenIframe(parentElement, data) {
    ...
    if (Util.isIE() && Util.getInternetExplorerVersion() < 9.0) {
      var iframe = document.createElement(
        '<iframe name="' + iframe_name + '" scr="about:blank" width="1" height="1"></iframe>'
      );
    } else {
  ...
  setSessionId(event, callback) {
    ...
    var is_new_session = this.isNewSession(
      event.get('timestamp'),
      this.getGlobalEventProperty('last_req')
    );
    if (is_new_session) {
