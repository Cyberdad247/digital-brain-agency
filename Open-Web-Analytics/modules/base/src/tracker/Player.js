...
class Player {
  ...
  showPlayerControls() {
    ...
    //add cursor
    var cursor =
      '<div id="owa-cursor"><img src="' +
      OWA_instance.getSetting('baseUrl') +
      '/modules/base/i/cursor2.png"></div>';
    ...
    // start player
    jQuery('#owa_player_start').bind('click', function (e) {
      that.play(e);
    });

    // pause player
    jQuery('#owa_player_stop').bind('click', function (e) {
      that.stop(e);
    });
  ...
  clickEventHandler(event) {
    ...
    jQuery('#' + id).css({
      position: 'absolute',
      left: event.click_x + 'px',
      top: event.click_y + 'px',
      'z-index': 89,
    });
