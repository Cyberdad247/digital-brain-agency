...
export const KPIDashboard = ({ department }: KPIDashboardProps) => {
  ...
  const calculateKPIs = () => {
    ...
    const completedTasks = departmentData.reduce(
      (sum, p) => sum + p.tasks.filter((t) => t.status === 'completed').length,
      0
    );
    const overdueTasks = departmentData.reduce(
      (sum, p) =>
        sum +
        p.tasks.filter((t) => new Date(t.dueDate) < new Date() && t.status !== 'completed').length,
      0
    );
